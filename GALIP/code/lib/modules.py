...
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        # NaN / Inf detection for discriminator loss
        try:
            finite = torch.isfinite(errD).all().item()
        except Exception:
            finite = False
        if not finite:
            print('\n===== NaN/Inf detected in discriminator loss =====', flush=True)
            print(f'errD: {errD}', flush=True)
            # Try to print loss components and tensor statistics to help debug
            try:
                print('errD_real:', errD_real, 'errD_fake:', errD_fake, 'errD_mis:', errD_mis, 'errD_MAGP:', errD_MAGP, flush=True)
            except Exception:
                pass
            try:
                def stats(t, name):
                    if t is None:
                        return f'{name}=None'
                    tn = t.detach().cpu()
                    return f"{name}: mean={tn.mean().item():.6g}, std={tn.std().item():.6g}, min={tn.min().item():.6g}, max={tn.max().item():.6g}, nan={torch.isnan(tn).any().item()}, inf={torch.isinf(tn).any().item()}"
                print(stats(CLIP_real, 'CLIP_real'))
                print(stats(real, 'real'))
                print(stats(real_emb, 'real_emb'))
                print(stats(real_feats, 'real_feats'))
                print(stats(CLIP_fake, 'CLIP_fake'))
                print(stats(fake, 'fake'))
                print(stats(fake_emb, 'fake_emb'))
                print(stats(fake_feats, 'fake_feats'))
            except Exception as e:
                print('Failed to compute tensor stats:', e, flush=True)
            # dump a small debug state with richer information
            try:
                import os
                debug_path = getattr(args, 'saved_debug_dir', None)
                if debug_path is None:
                    debug_path = osp.join(getattr(args, 'saved_images_dir', '.'), 'debug_nans')
                os.makedirs(debug_path, exist_ok=True)
                dump = {'step': step, 'errD': None}
                try:
                    dump['errD'] = float(errD.detach().cpu())
                except Exception:
                    dump['errD'] = None
                # attempt to save smaller tensors that help debugging
                try:
                    dump['errD_real'] = float(errD_real.detach().cpu())
                except Exception:
                    dump['errD_real'] = None
                try:
                    dump['errD_fake'] = float(errD_fake.detach().cpu())
                except Exception:
                    dump['errD_fake'] = None
                try:
                    dump['errD_mis'] = float(errD_mis.detach().cpu())
                except Exception:
                    dump['errD_mis'] = None
                try:
                    dump['errD_MAGP'] = float(errD_MAGP.detach().cpu())
                except Exception:
                    dump['errD_MAGP'] = None
                # Save small summaries (means) of key tensors
                try:
                    dump['CLIP_real_mean'] = CLIP_real.detach().cpu().mean().item()
                except Exception:
                    dump['CLIP_real_mean'] = None
                try:
                    dump['CLIP_fake_mean'] = CLIP_fake.detach().cpu().mean().item()
                except Exception:
                    dump['CLIP_fake_mean'] = None
                torch.save(dump, osp.join(debug_path, f'debug_errD_step{step}.pth'))
                # also try to save a tiny sample batch (CPU, first item) if available
                try:
                    sample_save = osp.join(debug_path, f'sample_step{step}.pth')
                    small = {}
                    if isinstance(real, torch.Tensor):
                        small['real0'] = real[0].detach().cpu()
                    if isinstance(fake, torch.Tensor):
                        small['fake0'] = fake[0].detach().cpu()
                    torch.save(small, sample_save)
                except Exception:
                    pass
            except Exception as e:
                print('Failed to save NaN debug state:', e, flush=True)
            raise ValueError('NaN/Inf in discriminator loss')

        # backward / step with optional AMP-aware gradient clipping
        if args.mixed_precision:
            ...
            if getattr(args, 'grad_clip', 0) and args.grad_clip > 0:
                clip_grad_norm_(list(netD.parameters()) + list(netC.parameters()), args.grad_clip)
        ...
        # NaN / Inf detection for generator loss
        try:
            finiteG = torch.isfinite(errG).all().item()
        except Exception:
            finiteG = False
        if not finiteG:
            print('\n===== NaN/Inf detected in generator loss =====', flush=True)
            print(f'errG: {errG}', flush=True)
            try:
                import os
                debug_path = getattr(args, 'saved_debug_dir', None)
                if debug_path is None:
                    debug_path = osp.join(getattr(args, 'saved_images_dir', '.'), 'debug_nans')
                os.makedirs(debug_path, exist_ok=True)
                torch.save({'step': step, 'errG': errG.detach().cpu()}, osp.join(debug_path, f'debug_errG_step{step}.pth'))
            except Exception as e:
                print('Failed to save NaN debug state:', e, flush=True)
            raise ValueError('NaN/Inf in generator loss')
        ...
        if args.mixed_precision:
            ...
            try:
                ...
                pass
            if getattr(args, 'grad_clip', 0) and args.grad_clip > 0:
                clip_grad_norm_(list(netG.parameters()), args.grad_clip)
        ...
        # accumulate losses
        with torch.no_grad():
            total_d_loss += errD.detach().float().item()
            total_g_loss += errG.detach().float().item()
            steps += 1
    ...
    if (args.multi_gpus==True) and (get_rank() != 0):
        ...
        loop.close()
    avg_g = total_g_loss / max(1, steps)
    avg_d = total_d_loss / max(1, steps)
    return avg_g, avg_d


@torch.no_grad()
def validate(dataloader, netG, netD, netC, text_encoder, image_encoder, args):
    device = args.device
    z_dim = args.z_dim
    netG.eval(); netD.eval(); netC.eval(); image_encoder.eval()
    total_g = 0.0
    total_d = 0.0
    steps = 0
    for data in dataloader:
        ...
        real_feats = netD(CLIP_real)
        pred_real, errD_real = predict_loss(netC, real_feats, sent_emb, negtive=False)
        mis_sent_emb = torch.cat((sent_emb[1:], sent_emb[0:1]), dim=0)
        _, errD_mis = predict_loss(netC, real_feats, mis_sent_emb, negtive=True)
        noise = torch.randn(real.size(0), z_dim, device=device)
        fake = netG(noise, sent_emb, eval=True)
        CLIP_fake, fake_emb = image_encoder(fake)
        fake_feats = netD(CLIP_fake)
        _, errD_fake = predict_loss(netC, fake_feats, sent_emb, negtive=True)
        ...
        text_img_sim = torch.cosine_similarity(fake_emb, sent_emb).mean()
        errG = -output.mean() - args.sim_w*text_img_sim
        total_d += errD.detach().float().item()
        total_g += errG.detach().float().item()
        steps += 1
    avg_g = total_g / max(1, steps)
    avg_d = total_d / max(1, steps)
    return avg_g, avg_d

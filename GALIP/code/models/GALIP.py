...
class CLIP_IMG_ENCODER(nn.Module):
    def __init__(self, CLIP, freeze: bool = True):
...
class CLIP_Adapter(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, cond_dim, k, s, p, map_num, CLIP, freeze_clip: bool = True):
        ...
        self.CLIP_ViT = CLIP_Mapper(CLIP, freeze=freeze_clip)
...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP, freeze_clip: bool = True):
        ...
        # Allow training CLIP backbone inside the adapter if not frozen by caller
        self.mapping = CLIP_Adapter(self.code_ch, self.mid_ch, self.code_ch, ngf*8, self.CLIP_ch, cond_dim+nz, 3, 1, 1, 4, CLIP, freeze_clip=freeze_clip)
        # build GBlocks
